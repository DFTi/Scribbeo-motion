...
-(void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
  ...
  CGFloat dist = (SQR(current.x - self.previousPoint2.x) + SQR(current.y - self.previousPoint2.y));
    if (dist > 1)
    {
...
//        dist = SQR(15);
//      }
        cacheBrushSize = (((1 - (dist / SQR(300))) * self.brushSize) + cacheBrushSize) / 2;
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.needsToRedraw) 
    {
        ...
        [newPath setLineWidth:cacheBrushSize];
...
{
   // SvgToBezier* stb = [[SvgToBezier alloc] initFromSVGPathNodeDAttr:<#(NSString *)#> rect:<#(CGRect)#>]
}

- (void)clearDrawing
{
    [self setPaths:nil];
    [self setPathColors:nil];
    [self setBufferImage:nil];
    [self setHasInput:NO];
    [self setNeedsDisplay];
}
